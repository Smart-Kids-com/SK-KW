...
export default async function Article({ params }) {
  ...
  `;
  const data = await fetchShopifyGraphQL(QUERY, { language: "EN", blogHandle: params.blogHandle, articleHandle: params.articleHandle });
  const art = data?.blog?.articleByHandle;
  if (!art) return <main style={{padding:24}}>Not found.</main>;
  return (
