...
// ========= SlideCard (نسخة محدثة مع hover effect) =========
function SlideCard({ s }) {
  ...
  const content = (
    <>
      <span style={{ position: "relative", paddingBottom: "100%", display: "block" }}>
        <img
      ...
      {(s.heading || s.subheading || s.button_label) && (
        <span style={{ padding: 16, display: "block" }}>
          {s.heading && (
            <span style={{ margin: 0, fontSize: "1.15rem", fontWeight: 700, display: "block" }}
          ...
          {s.subheading && (
            <span
              style={{ margin: "8px 0 0", color: "#64748b", lineHeight: 1.6, display: "block" }}
  ...
  // اختيار الـ style المناسب
  const cardStyle = isHovered ? hoveredCardStyle : baseCardStyle;
  ...
  // مهم: <Link> لازم يتقفل، ولو مفيش href بنرجّع <span>
  return href ? (
    <Link href={href} style={cardStyle} {...hoverProps}>
      {content}
    </Link>
  ) : (
    <span style={cardStyle} {...hoverProps}>{content}</span>
  );
}
                  ...
                  <div style={{ 
                    ...
                    {section.settings.cover_image && (
                      <div style={{ 
                        position: "relative", 
                        paddingBottom: 
      sectionId === "video_H4ERG" ? "100%" :          // المسلم الصغير (مربع)
      sectionId === "video_8eqz9" ? "177.78%" :       // قصص الأنبياء (Reel)
      sectionId === "video_U4WJHz" ? "177.78%" :      // القصص التدريجية (Reel)
      "100%", // default
    marginBottom: "1rem",
    borderRadius: 12,
    overflow: "hidden"
                    ...
                    <div className="mobile-only">
                      ...
                      <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: "1rem" }}>
                        <img
                          ...
                          style={{
                            width: "100%",
                            paddingBottom: "100%", // مربع مثالي يتكيف مع العرض
                            objectFit: "cover"
                          }}
                        />
                      </div>
