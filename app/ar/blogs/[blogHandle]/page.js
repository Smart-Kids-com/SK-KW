// app/ar/blogs/[blogHandle]/page.js
...
export default async function BlogList({ params }) {
  ...
  `;
  const data = await fetchShopifyGraphQL(QUERY, { language: "AR", handle: blogHandle, first: 20 });
    ...
    <main style={{ padding:24, maxWidth:1000, margin:"0 auto", direction:"rtl" }}>
      ...
      <div style={{ display:"grid", gap:16, gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))" }}>
        {items.map(a => (
          <Link
            key={a.handle}
            href={`/ar/blogs/${encodeURIComponent(blog.handle)}/${encodeURIComponent(a.handle)}`}
