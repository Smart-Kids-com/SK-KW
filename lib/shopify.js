...
// تطبيع نسخة الـAPI: يقبل "07-2025" أو "2025-07"
function normalizeApiVersion(v) {
  if (!v) return '2025-07';
  if (/^\d{4}-\d{2}$/.test(v)) return v;          // "2025-07"
  const m = v.match(/^(\d{2})-(\d{4})$/);          // "07-2025"
  if (m) return `${m[2]}-${m[1]}`;
  return v; // استخدم كما هو إن كانت صيغة صحيحة أخرى
}

const RAW_API_VERSION = process.env.SHOPIFY_API_VERSION || '2025-07';
const API_VERSION = normalizeApiVersion(RAW_API_VERSION);
