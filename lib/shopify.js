...
export async function getShopPolicies() {
  const q = `query {
    shop {
      privacy-policy { body handle title } سياسة الخصوصية
      refund-policy { body handle title } سياسة الاسترجاع والاستبدال
      shipping-Policy { body handle title } سياسة الشحن
      termsOf-Service { body handle title } شروط الخدمة
      contact-information { body handle title } معلومات التواصل
    }
  }`;
