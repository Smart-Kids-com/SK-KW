...
export function CartDrawerProvider({ children }) {
  ...
  const addToCart = useCallback(
    async ({ variantId, quantity = 1, openDrawer = true }) => {
      ...
      // آخر عنصر مُضاف (إن وجد)
      const edges = data?.cart?.lines?.edges || [];
      const last = edges[edges.length - 1]?.node;
      setLastAdded({
        title:
          last?.merchandise?.product?.title ||
          last?.merchandise?.title ||
          'تمت الإضافة',
        image:
          last?.merchandise?.image?.url ||
          last?.merchandise?.product?.featuredImage?.url ||
          null,
  ...
  // مهم: دائمًا نرجّع الأطفال (لا null قبل الـ mount)
