...
export default function Header() {
  ...
  const isAR = useMemo(() => pathname === "/ar" || pathname.startsWith("/ar/"), [pathname]);

  // تُعيد href مع الحفاظ على اللغة الحالية (بدون أي أزرار إضافية)
  ...
  return (
    <>
      <style jsx global>{`
        :root { --sk-header-shadow: 0 2px 8px rgba(0,0,0,.06); }
        @media (max-width: 640px) {
          .sk-actions a, .sk-actions a svg { width: 26px; height: 26px; }
          .sk-logo { height: 52px !important; }
        }
      `}</style>

      <header dir="rtl" role="banner" style={{ width: "100%", position: "sticky", top: 0, zIndex: 50, background: "#fff", boxShadow: "var(--sk-header-shadow)" }}>
        {/* شريط العرض */}
        <Link
          href={withLocale("/collections")}
          className="sk-topbar"
          style={{
            display: "block",
            background: "#3d0856",
        ...
        >
          ...
          >
            <img
              className="sk-logo"
              src="https://smart-kids.me/cdn/shop/files/Logo_smart_kids-of-header-tag.png?v=1756574543&width=600"
              alt="Smart Kids"
          ...
          <div className="sk-actions" style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <Link href={withLocale("/cart")} aria-label="عربة التسوق"><CartIcon /></Link>
          </div>
        </div>
      </header>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,.12)" }}>
          <strong style={{ fontSize: "1.05rem" }}>القائمة</strong>
          <button
            onClick={closeMenu}
            aria-label="إغلاق القائمة"
            style={{ background: "transparent", color: "#fff", border: "1px solid rgba(255,255,255,.35)", borderRadius: 8, padding: "6px 10px", cursor: "pointer" }}
          >
            إغلاق
          </button>
        </div>
